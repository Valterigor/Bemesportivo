...
if (root) {
  ...
  document.getElementById('fb-task-list').addEventListener('click',event=>{const complete=event.target.closest('[data-task-complete]'),edit=event.target.closest('[data-task-edit]'),remove=event.target.closest('[data-task-delete]'); if(complete){const task=tasks.find(item=>item.id===complete.dataset.taskComplete);if(!task)return;const previousDates=[...task.completedDates];task.completedDates=isDone(task,selected)?task.completedDates.filter(key=>key!==selected):[...task.completedDates,selected].slice(-180);if(!writeTasks()){task.completedDates=previousDates;render();window.dispatchEvent(new CustomEvent('meuCaminhoBe:feedback',{detail:{type:'warning',title:'Não foi possível salvar',message:'A tarefa foi mantida como estava. Libere espaço neste aparelho e tente novamente.'}}));return;}if(isDone(task,selected))window.dispatchEvent(new CustomEvent('meuCaminhoBe:feedback',{detail:{type:'success',title:'Tarefa concluída!',message:'Tudo certo. Seu acompanhamento da semana foi atualizado.',reward:'+5 XP',detail:'Rotina semanal atualizada'}}));} if(edit)openForm(tasks.find(item=>item.id===edit.dataset.taskEdit)); if(remove&&confirm('Excluir esta tarefa da sua agenda?')){const previousTasks=[...tasks];tasks=tasks.filter(item=>item.id!==remove.dataset.taskDelete);if(!writeTasks()){tasks=previousTasks;render();window.dispatchEvent(new CustomEvent('meuCaminhoBe:feedback',{detail:{type:'warning',title:'Não foi possível excluir',message:'A tarefa continua na sua agenda.'}}));}}});
  document.getElementById('fb-task-form').addEventListener('submit',event=>{event.preventDefault();if(!event.currentTarget.reportValidity())return;const data=new FormData(event.currentTarget);const previous=tasks.find(task=>task.id===editingId);const previousTasks=[...tasks];const task={id:editingId||crypto.randomUUID(),title:safeText(data.get('title')),date:String(data.get('date')),time:String(data.get('time')),category:String(data.get('category')),repeat:String(data.get('repeat')),reminder:String(data.get('reminder')),completedDates:previous?.completedDates||[],createdAt:previous?.createdAt||new Date().toISOString()};tasks=editingId?tasks.map(item=>item.id===editingId?task:item):[...tasks,task];selected=task.date;visibleMonth=parseDay(selected);visibleMonth.setDate(1);if(!writeTasks()){tasks=previousTasks;window.dispatchEvent(new CustomEvent('meuCaminhoBe:feedback',{detail:{type:'warning',title:'Não foi possível salvar',message:'O formulário continua aberto. Libere espaço neste aparelho e tente novamente.'}}));return;}closeForm();window.dispatchEvent(new CustomEvent('meuCaminhoBe:feedback',{detail:{type:'success',title:previous?'Tarefa atualizada!':'Tarefa salva!',message:'Tudo certo. Ela já aparece na sua agenda.'}}));});
  ...
  function sameApplicationKey(subscription,publicKey){const current=subscription?.options?.applicationServerKey;if(!current)return false;const expected=urlBase64ToUint8Array(publicKey),actual=new Uint8Array(current);return expected.length===actual.length&&expected.every((byte,index)=>byte===actual[index]);}
  async function syncPushSchedule(){if(!('Notification'in window)||Notification.permission!=='granted'||!('serviceWorker'in navigator))return false;try{const registration=await navigator.serviceWorker.ready;const subscription=await registration.pushManager.getSubscription();if(!subscription)return false;const response=await fetch('/api/routine-notifications/schedule',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({installationId:installId(),subscription,reminders:dueOccurrences()})});return response.ok;}catch(error){return false;}}
  async function enableNotifications(){const status=document.getElementById('fb-notification-status'),button=document.getElementById('fb-enable-notifications');if(!('Notification'in window)||!('PushManager'in window)||!window.isSecureContext){status.textContent='Este navegador não oferece push seguro. Os avisos aparecerão ao abrir o sistema.';return;}button.disabled=true;status.textContent='Ativando lembretes seguros…';try{const permission=await Notification.requestPermission();if(permission!=='granted'){status.textContent='Permissão não concedida. Você pode alterar isso nas configurações do navegador.';return;}const configResponse=await fetch('/api/routine-notifications/config');if(!configResponse.ok)throw new Error('config');const {publicKey}=await configResponse.json();const registration=await navigator.serviceWorker.ready;let subscription=await registration.pushManager.getSubscription();if(subscription&&!sameApplicationKey(subscription,publicKey)){await subscription.unsubscribe();subscription=null;}if(!subscription)subscription=await registration.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:urlBase64ToUint8Array(publicKey)});const response=await fetch('/api/routine-notifications/schedule',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({installationId:installId(),subscription,reminders:dueOccurrences()})});if(!response.ok)throw new Error('sync');root.querySelector('.fb-notification-panel').classList.add('enabled');status.textContent='Lembretes ativos neste navegador, inclusive com o site fechado.';button.textContent='Lembretes ativos';}catch(error){root.querySelector('.fb-notification-panel').classList.remove('enabled');status.textContent='A agenda continua funcionando. Os avisos com o site fechado ainda não estão disponíveis.';button.textContent='Tentar ativar lembretes';}finally{button.disabled=false;}}
  async function refreshNotificationState(){const status=document.getElementById('fb-notification-status'),button=document.getElementById('fb-enable-notifications');if(!('Notification'in window)||Notification.permission!=='granted')return;try{const configResponse=await fetch('/api/routine-notifications/config');if(!configResponse.ok)throw new Error('config');const {publicKey}=await configResponse.json();const registration=await navigator.serviceWorker.ready;const subscription=await registration.pushManager.getSubscription();if(!subscription||!sameApplicationKey(subscription,publicKey)){root.querySelector('.fb-notification-panel').classList.remove('enabled');status.textContent='Permissão concedida. Toque para concluir a ativação dos lembretes.';button.textContent='Concluir ativação';return;}const synchronized=await syncPushSchedule();if(!synchronized)throw new Error('sync');root.querySelector('.fb-notification-panel').classList.add('enabled');status.textContent='Lembretes ativos neste navegador, inclusive com o site fechado.';button.textContent='Lembretes ativos';}catch(error){root.querySelector('.fb-notification-panel').classList.remove('enabled');status.textContent='Os avisos aparecem com o sistema aberto. Ative o envio em segundo plano quando estiver disponível.';button.textContent='Tentar ativar lembretes';}}
  ...
  window.addEventListener('meuCaminhoBe:tasks-imported',()=>{tasks=readTasks();root.hidden=!profileReady();render();syncPushSchedule();});
