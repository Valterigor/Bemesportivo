...
async function requestFingerprint(request, clientId = '', runtime){
  const forwarded = request.headers.get('x-nf-client-connection-ip')
    || request.headers.get('cf-connecting-ip')
    || request.headers.get('x-forwarded-for')?.split(',')[0]
    || 'unknown';
  return runtime.fingerprint(`${forwarded}:${cleanId(clientId, 'device')}`);
}
...
async function writeState(state, runtime){
  state.updatedAt = new Date().toISOString();
  await runtime.write(state);
}
...
export default async function handler(request, runtime){
  ...
  if(request.method === 'POST' && action === 'comment'){
    ...
    const fingerprint = await requestFingerprint(request, body.clientId, runtime);
  ...
  if(request.method === 'POST' && action === 'comment-action'){
    ...
    const fingerprint = await requestFingerprint(request, clientId, runtime);
  ...
  if(request.method === 'POST' && action === 'palpite'){
    ...
    const fingerprint = await requestFingerprint(request, userId, runtime);
    ...
    const participantId = await requestFingerprint(request, userId, runtime);
    const cleanPalpite = {
      matchId,
      participantId,
